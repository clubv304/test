...
const TeamlistBet = async(req, res) => {
    let auth = req.cookies.auth;
    let phone = req.params.phone;
    let {pageno, limit } = req.body;
    ...
    let { token, password, otp, level,...userInfo } = user[0];
    const [user1] = await connection.query('SELECT code FROM users WHERE phone = ? ', [phone]);

    const inviteCode = user1[0].code;

    const [TeamlistBet] = await connection.query(`
    SELECT *
    FROM minutes_1
    WHERE invite = ?
    AND status != 0
    ...
    LIMIT ${pageno}, ${limit}
`, [inviteCode]);
    const [total_users] = await connection.query(`SELECT * FROM minutes_1 WHERE invite = ? AND status != 0`, [inviteCode]);
    return res.status(200).json({
        message: 'Success',
        status: true,
        datas11: TeamlistBet,
        page_total11: Math.ceil(total_users.length / limit)
    });
}
