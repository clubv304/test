...
const promotion = async (req, res) => {
    ...
    // today's direct subordinae
        ...
        const [recharge_total] = await connection.query('SELECT u.phone, SUM(r.money) AS total FROM users u LEFT JOIN recharge r ON u.phone = r.phone WHERE r.status=1 and u.invite = ? GROUP BY u.phone', [userInfo.code]);
        
		 const [recharge_today] = await connection.query('SELECT u.phone, SUM(r.money) AS total , count(8) as totall FROM users u LEFT JOIN recharge r ON u.phone = r.phone WHERE r.status=1 and DATE(r.today) = CURDATE() and  u.invite = ? GROUP BY u.phone', [userInfo.code]);
		...
		const [withdraw_week_total] = await connection.query(`
  SELECT u.phone, SUM(COALESCE(r_k3.money, 0) + COALESCE(r_5d.money, 0) + COALESCE(m1.money, 0)) AS total
  FROM users u
  LEFT JOIN result_k3 r_k3 ON u.phone = r_k3.phone AND WEEK(r_k3.today, 1) = WEEK(CURDATE(), 1)
  LEFT JOIN result_5d r_5d ON u.phone = r_5d.phone AND WEEK(r_5d.today, 1) = WEEK(CURDATE(), 1)
  LEFT JOIN minutes_1 m1 ON u.phone = m1.phone AND WEEK(m1.today, 1) = WEEK(CURDATE(), 1)
  WHERE u.invite = ?
  GROUP BY u.phone
`, [userInfo.code]);
...
if(userInfo.bonus<totalbonus){
    ...
    return res.status(200).json({
        ...
        invite: {
           ...
           f1_week: withdraw_week_total.reduce((sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0), 0),
            roses_f1: recharge_total.reduce((sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0), 0),
           
		   roses_today: recharge_today.reduce((sum, entry) => sum + (entry.total ? parseInt(entry.total) : 0), 0),
			roses_today1: recharge_today.reduce((sum, entry) => sum + (entry.totall ? parseInt(entry.totall) : 0), 0),
